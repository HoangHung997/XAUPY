...
def exchange(file, message_type: str, payload: dict | None = None) -> dict:
    ...
    line = file.readline()
    if not line:
        raise RuntimeError(f"engine closed during {message_type}")
...
def main() -> int:
    ...
    try:
        ...
        assert schema["payload"]["config_schema"]["field_count"] >= 100
        assert schema["payload"]["execution_enabled"] is False

        defaults = exchange(file, "config_defaults_get")
        profile = defaults["payload"]["profile"]
        validate = exchange(file, "config_validate", {"profile": profile})
        assert validate["payload"]["valid"] is True
        assert validate["payload"]["execution_enabled"] is False

        profile["execution"]["allow_real_account"] = True
        rejected = exchange(file, "config_validate", {"profile": profile})
        assert rejected["payload"]["valid"] is False
        assert rejected["payload"]["execution_enabled"] is False

        shutdown = exchange(file, "shutdown", {"reason": "task004-ci-smoke"})
        ...
        print("PASS: Task004 packaged Engine config schema/defaults/validation smoke test")
        return 0
    finally:
        if sock is not None:
            sock.close()
